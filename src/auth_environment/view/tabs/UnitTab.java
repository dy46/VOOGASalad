...
public class UnitTab extends ElementTab {
    ...
    public UnitPicker setUpUnitPicker () {
        UnitPicker up = new UnitPicker(getLabelsBundle().getString("editLabel"),
                                       this.myElementTabModel.getUnitFactory().getUnitLibrary()
                                               .getUnits());
    ...
    private void addTextFields (GridPane gp) {
        for (String s : Arrays.asList(getLabelsBundle().getString("unitTextProperties")
            ...
            getCreator().createTextLabels(gp, s, getIndex(), 1,
                                          Double.parseDouble(getDimensionsBundle()
                                                  .getString("rowConstraintSize")));
        ...
        for (String s : Arrays.asList(getLabelsBundle().getString("unitComboProperties")
            ...
            getCreator().createTextLabels(gp, s, getIndex(), 1,
                                          Double.parseDouble(getDimensionsBundle()
                                                  .getString("rowConstraintSize")));
            ComboBox<String> cb =
                    getCreator().createStringComboBox(gp, Arrays.asList(getLabelsBundle()
                            .getString("elementTypes").split(getLabelsBundle().getString("regex"))),
                                                      getIndex(), 2);
    ...
    private void createUnit (String name,
        ...
        try {
            ...
            Unit unit = myUnitFactory.createUnit(name,
                                                 strComboMap
                                                         .get(getLabelsBundle()
                                                                 .getString("unitComboProperties"))
                                                         .getValue(),
                                                 map, projectiles, atu, ata);
    ...
    private void addTextComboButtonTrio (GridPane gp,
                                         ...
                                         String buttonText,
                                         String labelText) {
        getCreator().createTextLabels(gp, labelText, getIndex(), 1, Double
