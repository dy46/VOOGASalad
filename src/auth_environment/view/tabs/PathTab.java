...
public class PathTab extends Tab implements IWorkspace {
		...
		myPathPane.setOnMouseClicked(e -> {
			if (e.isControlDown()) {
				if(drawingIndex == 0) {
					...
					Position spawn = addSpawnPoint(e.getX(), e.getY());
					if(spawn == null){
						addUnrestrictedSpawn(e.getX(), e.getY());
					}
				}
				drawMap();
			}
	...
	private void addUnrestrictedSpawn(double x, double y) {
		myPathTabModel.addUnrestrcitedSpawn(x, y);
	}
	...
	private Position addSpawnPoint(double x, double y) {
		return myPathTabModel.addNewSpawn(x, y);
	}
