...
public class PathTab extends Tab implements IWorkspace {
    ...
    private void setupBorderPane () {
        ...
        myBorderPane.setPrefSize(Double
                .parseDouble(myDimensionsBundle.getString("defaultBorderPaneWidth")),
                                 Double.parseDouble(myDimensionsBundle
                                         .getString("defaultBorderPaneHeight")));
    ...
    private Node buildCenter () {
        ...
        center.getChildren().addAll(
                                    ...
                                    myPathPane);
        addClickHandlers();
        myPathPane.setOnKeyPressed(e -> setUpGClicked(e.getCode()));
        return center;
    }
    ...
    private void buildLevelComboBox () {
        ...
        if (!myAuthModel.getIAuthEnvironment().getLevels().isEmpty()) {
            ...
            myLevelComboBox.setOnAction(event -> {
                String selectedItem =
                        ((ComboBox<String>) event.getSource()).getSelectionModel()
                                .getSelectedItem();
    ...
    private VBox buildDefaultVBox () {
        return myNodeFactory
                .buildVBox(Double.parseDouble(myDimensionsBundle.getString("defaultVBoxSpacing")),
    ...
    private HBox buildFirstRowButtons () {
        HBox hb0 =
                myNodeFactory.buildHBox(Double
                        .parseDouble(myDimensionsBundle.getString("defaultHBoxSpacing")), 0);
        myPathWidthField =
                myNodeFactory.buildTextFieldWithPrompt(myNamesBundle.getString("pathWidthPrompt"));
        myPathWidthField.setOnAction(e -> submitPathWidth(myPathWidthField));
        Button submitBranchButton =
                myNodeFactory.buildButton(myNamesBundle.getString("submitBranchButtonLabel"));
        submitBranchButton.setOnAction(e -> submitBranch());
        Button help =
                myNodeFactory.buildButtonWithEventHandler(myNamesBundle.getString("openHelpItem"),
                                                          e -> {
                                                              BrowserWindowDelegate browser =
                                                                      new BrowserWindowDelegate();
                                                              browser.openWindow(myNamesBundle
                                                                      .getString("helpMenuLabel"),
                                                                                 myURLSBundle
                                                                                         .getString("helpURL"),
                                                                                 Double.parseDouble(myDimensionsBundle
                                                                                         .getString("helpWidth")),
                                                                                 Double.parseDouble(myDimensionsBundle
                                                                                         .getString("helpHeight")));
                                                          });
    ...
    private void drawTerrains () {
        if (!myAuthEnvironment.getPlacedUnits().isEmpty()) {
            myAuthEnvironment.getPlacedUnits().stream().forEach(e -> {
                UnitView temp =
                        new UnitView(e, e.toString() +
                                        myNamesBundle.getString("defaultImageExtension"));
                myTerrains.add(temp);
            });
    ...
    private void addBoundLine (double startX,
                               double startY,
                               double endX,
                               double endY,
                               Branch branch) {
