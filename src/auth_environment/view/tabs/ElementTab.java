...
public class ElementTab extends UnitTab {
    ...
    public UnitPicker setUpUnitPicker () {
        UnitPicker up =
                new UnitPicker(getLabelsBundle().getString("editLabel"),
    ...
    public void refresh () {
        ...
        setIndex(1);
        affectorNames =
                this.myElementTabModel.getAffectoryFactory().getAffectorLibrary()
                        .getAffectorNames();
    ...
    private void addTextFields (GridPane newTableInfo, FrontEndCreator creator) {
        for (String s : Arrays.asList(getLabelsBundle().getString("unitTextProperties")
                .split(getLabelsBundle().getString("regex")))) {
            creator.createTextLabels(newTableInfo, s, getIndex(), 1, Double
                    .parseDouble(getDimensionsBundle().getString("rowConstraintSize")));
            strTextMap.put(s, creator.createTextField(newTableInfo, getIndex(), 2));
            iterateIndex();
        }
        for (String s : Arrays.asList(getLabelsBundle().getString("unitComboProperties")
                .split(getLabelsBundle().getString("regex")))) {
            creator.createTextLabels(newTableInfo, s, getIndex(), 1, Double
                    .parseDouble(getDimensionsBundle().getString("rowConstraintSize")));
            ComboBox<String> cb =
                    creator.createStringComboBox(newTableInfo,
                                                 Arrays.asList(getLabelsBundle()
                                                         .getString("elementTypes")
                                                         .split(getLabelsBundle()
                                                                 .getString("regex"))),
                                                 getIndex(), 2);
    ...
    public void createNewElement () {
        ...
        for (String str : strTextMap.keySet()) {
            ...
            String[] strings =
                    strTextMap.get(str).getText().trim()
                            .split(getLabelsBundle().getString("regex"));
        ...
        Unit unit =
                myUnitFactory.createUnit(name, strComboMap.get("UnitType").getValue(),
                                         strToDoubleMap, projectiles, atu, ata);
    ...
    public void updateMenu (Unit unit) {
        ...
        strTextMap.get("Speed").setText(unit.getProperties().getVelocity().getSpeed() + "");// check
                                                                                            // these
                                                                                            // 3
    ...
    public void addFields (GridPane gp, FrontEndCreator creator) {
        ...
        addTextComboButtonTrio(gp, creator, unitNames, myProjectiles,
                               getLabelsBundle().getString("childButton"),
                               getLabelsBundle().getString("childText"));
        addTextComboButtonTrio(gp, creator, affectorNames, affectorsToUnit,
                               getLabelsBundle().getString("affectorsButton"),
                               getLabelsBundle().getString("affectorsText"));
        addTextComboButtonTrio(gp, creator, affectorNames, affectorsToApply,
                               getLabelsBundle().getString("applyButton"),
                               getLabelsBundle().getString("applyText"));
    }

    private void addTextComboButtonTrio (GridPane gp,
                                         FrontEndCreator creator,
                                         List<String> listOfNames,
                                         List<ComboBox<String>> comboList,
                                         String buttonText,
                                         String labelText) {
        creator.createTextLabels(gp, labelText, getIndex(), 1, Double
                .parseDouble(getDimensionsBundle().getString("rowConstraintSize")));
        ...
        button.setOnAction(e -> addNewComboBox(currentInt, gp, button, cbox, 2, comboList,
                                               listOfNames, creator));
        iterateIndex();
    }

    private void addNewComboBox (int row,
                                 GridPane gp,
                                 Button button,
                                 ComboBox<String> cbox,
                                 int col,
                                 List<ComboBox<String>> list,
                                 List<String> names,
                                 FrontEndCreator creator) {
        if (cbox.getValue() != null) {
            int newcol = col + 1;
            gp.getRowConstraints().add(new RowConstraints(Double
                    .parseDouble(getDimensionsBundle().getString("rowConstraintSize"))));
