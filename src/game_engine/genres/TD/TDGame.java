...
//		List<List<AffectorTimeline>> timelines = myEnemys.stream().map(e -> e.getTimelines()).collect(Collectors.toList());
//		List<List<AffectorTimeline>> timelines2 = myProjectiles.stream().map(e -> e.getTimelines()).collect(Collectors.toList());
//		List<List<Wave>> waves = myLevels.stream().map(l -> l.getWaves()).collect(Collectors.toList());
//		List<List<AffectorTimeline>> towerTimelines = myTowers.stream().map(e -> e.getTimelines()).collect(Collectors.toList());
//		for(List<Wave> wList : waves){
//			for(Wave w: wList){
//				for(Enemy e : w.getEnemies()){
//					for(AffectorTimeline t : e.getTimelines()){
//						for(Affector a : t.getAffectors()){
//							a.setWorkspace(this);
//						}
//					}
//				}
